...
| `zalo/responder.js` | Conversation history (`_fetchHistory`) for follow-up questions only fetched when `isDirect` — @mention conversations in a GROUP always got `history=[]`, so follow-up questions ("vậy cái đó thì sao", single-word replies) lost all context and confused the RAG answer | Removed the `isDirect` gate — `_fetchHistory(senderUid)` already scopes by sender, so it's safe to fetch for both 1:1 and group @mentions |
