...
export default function KnowledgeBasePage() {
  ...
  async function send() {
    ...
    try {
      ...
      setMessages(m => [...m, { role: 'ai', text: t('kb.errorRetry') }]);
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <main className="flex-1 overflow-auto min-w-0">
        ...
        <div className="sticky top-0 z-10 bg-gray-50/90 backdrop-blur border-b border-gray-200 px-4 pt-18 pb-3 md:pt-4 md:px-8 md:pb-4">
          <h1 className="text-lg font-bold text-gray-900">{t('kb.title')}</h1>
          <p className="text-xs text-gray-500 mt-0.5">{t('kb.subtitle')}</p>
        </div>
        ...
        <div className="px-4 pb-8 md:px-8 pt-6 space-y-6 max-w-5xl mx-auto">
          ...
          <div className="flex flex-col lg:flex-row gap-4 min-w-0">
            ...
            <div className="lg:w-56 shrink-0 bg-white border border-gray-200 rounded-xl p-4">
              <p className="text-xs font-semibold text-gray-500 mb-3 uppercase tracking-wide">
                {t('kb.indexedDocs')} ({files.length} {t('kb.files')})
              </p>
              {files.length === 0 && (
                ...
                {files.map(f => (
                  <li key={f.file_name} className="flex items-start gap-2">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[10px] text-gray-400">{f.chunks} {t('kb.chunks')}</p>
                    </div>
                  </li>
                ))}
              </ul>
            ...
            <div className="flex-1 min-w-0 bg-white border border-gray-200 rounded-xl flex flex-col" style={{ height: 440 }}>
              <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
                {messages.length === 0 && (
                  <p className="text-xs text-gray-400 text-center mt-8">{t('kb.emptyChat')}</p>
                )}
                {messages.map((m, i) => (
                ...
                {loading && (
                  <div className="flex justify-start">
                    <div className="bg-gray-100 rounded-xl px-3 py-2 text-sm text-gray-400">{t('kb.searching')}</div>
                  </div>
                )}
                <div ref={bottomRef} />
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-4">
              <p className="text-xs font-semibold text-gray-500 mb-1 uppercase tracking-wide">{t('kb.topQuestionsTitle')}</p>
              <p className="text-[10px] text-gray-400 mb-3">{t('kb.topQuestionsSub')}</p>
              {!stats?.topQuestions.length && <p className="text-xs text-gray-400">{t('common.noData')}</p>}
              <ul className="space-y-2">
            ...
            {/* Unanswered */}
            <div className="bg-white border border-gray-200 rounded-xl p-4">
              <p className="text-xs font-semibold text-gray-500 mb-1 uppercase tracking-wide">{t('kb.unansweredTitle')}</p>
              <p className="text-[10px] text-gray-400 mb-3">{t('kb.unansweredSub')}</p>
              {!stats?.unanswered.length && <p className="text-xs text-gray-400">{t('kb.unansweredGood')}</p>}
              <ul className="space-y-2">
            ...
            {/* Doc usage */}
            <div className="bg-white border border-gray-200 rounded-xl p-4">
              <p className="text-xs font-semibold text-gray-500 mb-1 uppercase tracking-wide">{t('kb.docUsageTitle')}</p>
              <p className="text-[10px] text-gray-400 mb-3">{t('kb.docUsageSub')}</p>
              {!stats?.docUsage.length && <p className="text-xs text-gray-400">{t('common.noData')}</p>}
              <ul className="space-y-2">
